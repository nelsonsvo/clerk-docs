...
Completes a email link verification flow once we've reached the email link results URL.
...
| Type | Description |
| --- | --- |
| `Promise<unknown>` | This method will throw a [`EmailLinkError`](/docs/custom-flows/email-links) if the email link verification failed or the link expired. Check the error's `code` property for details. |
